...
exports.aggregateAiUsage = exports.triggerWhatsappBatch = exports.scheduledWhatsappBatchProcess = exports.onWhatsappMessageCreated = exports.notifyAdminOnSupportTicket = exports.processWhatsappEvent = exports.syncWhatsappGroups = exports.evolutionWebhook = exports.getWhatsappEvents = exports.setWhatsappWebhook = exports.deleteWhatsappInstance = exports.logoutWhatsappInstance = exports.getWhatsappQrCode = exports.createWhatsappInstance = exports.listWhatsappInstances = void 0;
...
const whatsappTransactionPrompt_1 = require("./whatsappTransactionPrompt");
const aiTaskPlanner_1 = require("./aiTaskPlanner");
...
async function evolutionProxy(method, path, body = null, timeoutMs = 20000) {
    ...
    if (!apiUrl || !apiKey) {
        throw new https_1.HttpsError("failed-precondition", "Evolution API not configured");
    }
    ...
    try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
            ...
            throw new https_1.HttpsError("internal", `API returned non-JSON response (${response.status})`);
        }
        ...
        if (error.name === "AbortError") {
            ...
            throw new https_1.HttpsError("deadline-exceeded", `Timeout communicating with Evolution API after ${timeoutMs}ms`);
        }
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError("internal", `Error communicating with Evolution API: ${error.message}`);
    }
}
 ...
 */
exports.listWhatsappInstances = (0, https_1.onCall)({
 ...
 */
exports.createWhatsappInstance = (0, https_1.onCall)({
    secrets: ["EVOLUTION_API_URL", "EVOLUTION_API_KEY"],
}, async (request) => {
    await ensureAdmin(request.auth);
    const { instanceName } = request.data;
    if (!instanceName) {
        throw new https_1.HttpsError("invalid-argument", "instanceName is required");
    }
 ...
 * Proxy: Get QR Code
 */
exports.getWhatsappQrCode = (0, https_1.onCall)({
    secrets: ["EVOLUTION_API_URL", "EVOLUTION_API_KEY"],
}, async (request) => {
    await ensureAdmin(request.auth);
    const { instanceName } = request.data;
 ...
 */
exports.logoutWhatsappInstance = (0, https_1.onCall)({
    secrets: ["EVOLUTION_API_URL", "EVOLUTION_API_KEY"],
}, async (request) => {
    await ensureAdmin(request.auth);
    const { instanceName } = request.data;
 ...
 */
exports.deleteWhatsappInstance = (0, https_1.onCall)({
    secrets: ["EVOLUTION_API_URL", "EVOLUTION_API_KEY"],
}, async (request) => {
    await ensureAdmin(request.auth);
    const { instanceName } = request.data;
 ...
 * Proxy: Set Webhook
 */
exports.setWhatsappWebhook = (0, https_1.onCall)({
 ...
 */
exports.getWhatsappEvents = (0, https_1.onCall)(async (request) => {
    await ensureAdmin(request.auth);
    const db = (0, firestore_2.getFirestore)();
...
function verifySignature(rawBody, signature, secret) {
    if (!secret)
        return true; // Insecure mode for dev
    if (!signature)
        return false;
 ...
 */
exports.evolutionWebhook = (0, https_1.onRequest)({
    ...
    try {
        ...
        await eventRef.set({
            ...
            occurredAt: payload.createdAt || firestore_2.FieldValue.serverTimestamp(),
        }, { merge: true });
 ...
 */
exports.syncWhatsappGroups = (0, https_1.onCall)({
    secrets: ["EVOLUTION_API_URL", "EVOLUTION_API_KEY"],
}, async (request) => {
    const { instanceName } = request.data;
    if (!instanceName) {
        throw new https_1.HttpsError("invalid-argument", "Nome da instância é obrigatório.");
    }
    const db = (0, firestore_2.getFirestore)();
    try {
        ...
        for (const group of groups) {
            if (!group.id || (!group.subject && !group.name))
                continue;
        ...
        throw new https_1.HttpsError("internal", error.message || "Erro desconhecido na sincronização");
    }
});
/**
 ...
 */
exports.processWhatsappEvent = (0, firestore_1.onDocumentCreated)("whatsapp_webhook_events/{eventId}", async (event) => {
    ...
    try {
        ...
        if (isGroup && !groupName) {
            ...
            if (!groupName && eventData.instanceId) {
                try {
                    const groupInfoRes = await evolutionProxy("GET", `/group/findGroupInfos/${eventData.instanceId}?groupJid=${remoteJid}`, null, 15000);
        ...
        const messageDoc = {
            ...
            timestamp: data.messageTimestamp ? new Date(data.messageTimestamp * 1000) : firestore_2.FieldValue.serverTimestamp(),
...
exports.notifyAdminOnSupportTicket = (0, firestore_1.onDocumentCreated)("support_tickets/{ticketId}", async (event) => {
    const db = (0, firestore_2.getFirestore)();
 ...
 */
exports.onWhatsappMessageCreated = (0, firestore_1.onDocumentCreated)({
...
async function processPendingWhatsappBatches() {
    ...
    // Group by remoteJid
    const messagesByJid = {};
    pendingMessagesSnap.docs.forEach((doc) => {
        const data = doc.data();
    ...
    for (const [remoteJid, messages] of Object.entries(messagesByJid)) {
        ...
        const hasNewMessages = messages.some((m) => m.extracted === "pending_batch");
        if (!hasNewMessages) {
            continue;
        }
        ...
        const combinedText = messages.map((m) => `[${m.timestamp?.toDate ? m.timestamp.toDate().toISOString() : m.timestamp}] ${m.pushName || 'User'}: ${m.text}`).join("\n");
        try {
            // 1. Plan AI Task
            const plan = await (0, aiTaskPlanner_1.planAiTask)("whatsapp_batch_extraction", remoteJid, {
            ...
            const prompt = whatsappTransactionPrompt_1.WHATSAPP_TRANSACTION_EXTRACTION_PROMPT.replace("{{messageText}}", combinedText);
            const runResult = await (0, aiTaskPlanner_1.executeAiTask)(plan, prompt);
            ...
            const extractionRef = db.collection(whatsappDomain_1.WHATSAPP_COLLECTIONS.whatsappExtractedTransactions).doc();
            const extractionRecord = (0, whatsappDomain_1.createWhatsappExtractedTransactionRecord)({
                ...
                lineage: {
                    ...
                    batchMessageIds: messages.map((m) => m.id) // Track all messages that went into this extraction
                }
            });
            ...
            messages.forEach((m) => {
                ...
                batch.update(msgRef, {
                    ...
                    batchProcessedAt: firestore_2.FieldValue.serverTimestamp(),
 ...
 */
exports.scheduledWhatsappBatchProcess = (0, scheduler_1.onSchedule)({
 ...
 */
exports.triggerWhatsappBatch = (0, https_1.onCall)({
    secrets: ["GEMINI_API_KEY", "DEEPSEEK_API_KEY"]
}, async (request) => {
    await ensureAdmin(request.auth);
...
const finops_1 = require("./finops");
Object.defineProperty(exports, "aggregateAiUsage", { enumerable: true, get: function () { return finops_1.aggregateAiUsage; } });
