...
export function buildBackfillBucketKey(value) {
  const isoValue = normalizeIsoString(value);
  if (!isoValue) return null;
  return isoValue.slice(0, 10);
}

export function buildHistoryCutoffIso(targetDate) {
...
function updateBackfillQueueContact(job, contactId, updater) {
  const nextQueue = (job.queueContacts || []).map((entry) => (
...
export function createWhatsappInstanceRecord(payload = {}, ownershipContext = {}) {
...
export function createWhatsappWebhookEventRecord(payload = {}, ownershipContext = {}) {
...
export function createHistoryBackfillJobRecord(payload = {}, ownershipContext = {}) {
...
export function getNextBackfillQueueContact(job = {}) {
  return (job.queueContacts || []).find((entry) => entry.status === "pending") || null;
}

export function markBackfillContactAttempt(job = {}, contactId, metadata = {}) {
...
export function markBackfillContactCompleted(job = {}, contactId, metadata = {}) {
  return updateBackfillQueueContact(job, contactId, (entry) => ({
    ...entry,
    status: "completed",
...
export function markBackfillContactFailed(job = {}, contactId, metadata = {}) {
  return updateBackfillQueueContact(job, contactId, (entry) => ({
    ...entry,
    status: "failed",
...
export function createTxtImportJobRecord(payload = {}, ownershipContext = {}) {
...
export function createMessageDispatchJobRecord(payload = {}, ownershipContext = {}) {
